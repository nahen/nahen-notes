...
### メタデータの取得が面倒
この方法には欠点がある。「抜粋した文章をSRSに入れる」という作業が重すぎるのだ。たとえば物理的な本の場合、本に書かれた文章をメモ帳に書き起こす必要がある。

とはいえ、文字の書き起こし作業自体はそこまで苦ではない。自分の手で書くのが面倒であれば、写真を撮って画像のまま貼り付けるなり、その画像をOCR[^ocr]で読み取ってテキストを抽出する手もある。

[^ocr]: Optical character recognition。光学式文字認識。撮影した画像にある文字の形状や輪郭を読み取り、対応する文字データへと自動的に変換する機能。いまではiPhoneでもAndroidでも、カメラにOCR機能がついている。

問題は、メタデータも一緒に取り入れたい場合だ。書き起こした文章すべてにこれらのデータをひとつずつ付与していくのは、地味な割に骨が折れる。
...
「気になった文章を抜粋する」作業では、読者が文章を読むたびに「この文章は重要か否か」を判断する必要がなる。この判断自体はそこまで負担ではない。

しかし、ここに「重要であれば文章を書き起こしてSRSに加える」という作業が合わさると、重要だと判断すること自体が重荷になる。
...
ただ、[[💭Ankiで本を読む試みはうまくいかない？]]とする声もある。